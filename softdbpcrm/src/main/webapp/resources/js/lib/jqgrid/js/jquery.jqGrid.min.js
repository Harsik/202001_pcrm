...
j=$("<div class='ui-jqgrid ui-widget ui-widget-content ui-corner-all'></div>");$(j).attr({id:"gbox_"+this.id,dir:g}).insertBefore(h),$(h).attr("id","gview_"+this.id).appendTo(j),$("<div class='ui-widget-overlay jqgrid-overlay' id='lui_"+this.id+"'></div>").insertBefore(h),$("<div class='loading ui-state-default ui-state-active' id='load_"+this.id+"'><img src='/resources/images/loading.gif' alt='loading' class='img_loading'></div>").insertBefore(h),$(this).attr({cellspacing:"0",cellpadding:"0",border:"0",role:"presentation","aria-multiselectable":!!this.p.multiselect,"aria-labelledby":"gbox_"+this.id});var
...
b="show"===a.method?!0:!1;switch(a.loadtype){case"disable":break;case"enable":$("#load_"+$.jgrid.jqID(this.p.id)).toggle(b);break;case"block":$("#lui_"+$.jgrid.jqID(this.p.id)).toggle(b),$("#load_"+$.jgrid.jqID(this.p.id)).toggle(b)}})},getColProp:function(a){var
