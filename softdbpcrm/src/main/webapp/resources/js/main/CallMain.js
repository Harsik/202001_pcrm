...
var gConferDept ="평택시민원상담콜센터"; // 바로가기 3자통화 부서 
...
//	}
	...
	else if(calltype=="location"||calltype=="ssnumber")
	{
		...
		if( ( $("#OUTDIAL").val() !="" || $("#tfCustCelPhoneNum").val() !="") && (gCallStartTime != null && gCallReleaseTime != null))
		{
			...
			fnConsult("AUTH", "3000" , $("#USERID").val(), sendUEI); 
		}
		else
		{
...
//			var encText = encodeURI($("#searchTotal").val());
			...
			var paramURL = "http://counsel.pyeongtaek.go.kr:7080/search.s";
			...
			//searchWin.document.getElementById("query").value="평택";
...
{	
	...
	$("#selMainActtypecd").val("010000");		//평택시청 - "직접상담"으로 셋팅
...
function minwonAlrmSendSMS(reqview){
	...
	var mainDpt=["",""];
	var subDpt=["",""];
	var prefixMsgToCust="안녕하십니까? 평택시민원상담콜센터입니다. 접수내용이  ";
...
function fnSetTransferInfo(objInfo)
{
	...
	if (actTypeCd == "030100") { // 민원이관
		if (objInfo.GUBUN == 'M') { // 주관부서
			...
			$("#tfTransfDept_01").val((objInfo.DEPT_NM).replace("평택시청 ", "") + " > " + objInfo.USR_NM);
			...
			$("#tfTransfDept_02").val((objInfo.DEPT_NM).replace("평택시청 ", "") + " > " + objInfo.USR_NM);
...
function showFindDiquest(){
	...
	var left = (screen.width - width) / 2;
	var encText = encodeURI($("#searchTotal").val());
	var paramURL = "http://counsel.pyeongtaek.go.kr:7080/search.s?pQuery=" + encText + "&pQuert_tmp=" + encText;
	var option = "width=" + width + ", height=" + height
		+ ", toolbar=no, directories=no, scrollbars=yes, location=no, resizable=no, status=no,menubar=no, top="
		+ top + ",left=" + left +"";
                        ...
                        response(
                            $.map(data, function(item) {
                                return { 
                                    ...
                                    value: (item.ORGFULLNAME).replace("평택시청 ", "")+" > "+item.DISPLAYNAME,
