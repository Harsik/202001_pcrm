...
function fnSetTransInfo(objInfo)
{
	...
	if (actTypeCd == "030100") { // 민원이관
		if (objInfo.GUBUN == "M") { // 주관부서
			...
			$("#cmscsp_cnslMainUsr").val(objInfo.DEPT_NM.replace("평택시청 ", "") + " > " + objInfo.USR_NM);
			...
			$("#cmscsp_cnslSubUsr").val(objInfo.DEPT_NM.replace("평택시청 ", "") + " > " + objInfo.USR_NM);			
			...
			var szLength = data.length;
				if (szLength > 0) {
					...
					if(data[0].ORG_ID){
						...
						$("#cmscsp_cnslMainUsr").val(data[0].ORG_FUL_NM.replace("평택시청 ", "") +" > "+ name);
					...
					if (szLength > 1) {
						...
						$("#cmscsp_cnslSubUsr").val(data[1].ORG_FUL_NM.replace("평택시청 ", "") +" > "+data[1].AFFS_USR_NM);
